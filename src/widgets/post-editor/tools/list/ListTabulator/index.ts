...
import type { ItemMeta, OrderedListItemMeta, UnorderedListItemMeta } from '../types/ItemMeta'
import type { ListItem, ListParams } from '../types/ListParams'
...
export default class ListTabulator<Renderer extends ListRenderer> {
    ...
    public pasteHandler(element: PasteEvent['detail']['data']) {
        ...
        const getPastedItems = (parent: Element): ListItem[] => {
            const children = Array.from(parent.querySelectorAll(`li`))
    ...
    private mergeItemWithPrevious(item: ItemElement): void {
        ...
        const targetChildWrapper =
            getItemChildWrapper(targetForChildItems) ?? this.renderer.renderWrapper(false)
    ...
    private renderItem(itemContent: ListItem['content'], meta?: ListItem['meta']): ItemElement {
        ...
        switch (true) {
            ...
            default:
                throw new Error('Checklist is unsupported')
        }
    }
